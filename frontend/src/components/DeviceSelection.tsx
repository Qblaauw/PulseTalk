    ...
    return (
      <div className="p-4 space-y-4">
        <div className="animate-pulse">
          <div className="mb-4 h-4 w-1/3 bg-[var(--pt-border)] [border-radius:3px]"></div>
          <div className="mb-3 h-10 bg-[var(--pt-border)] [border-radius:3px]"></div>
          <div className="h-10 bg-[var(--pt-border)] [border-radius:3px]"></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-[var(--pt-text)]">Audio devices</h4>
        <div className="flex items-center space-x-2">
          <button
            onClick={handleRefresh}
            disabled={refreshing || disabled}
            className="pt-focus-ring inline-flex h-8 w-8 items-center justify-center p-0 text-sm font-medium text-[var(--pt-text-secondary)] transition-colors [border-radius:3px] hover:bg-[var(--pt-surface-hover)] disabled:pointer-events-none disabled:opacity-50"
      ...
      {error && (
        <div className="border border-[var(--pt-error)] bg-[var(--pt-error-wash)] p-3 text-sm text-[var(--pt-error)] [border-radius:3px]">
          {error}
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Mic className="h-4 w-4 text-[var(--pt-accent)]" />
            <Label htmlFor="mic-selection" className="text-sm font-medium text-[var(--pt-text)]">
              Microphone
            </Label>
          </div>
          <Select
            ...
            disabled={disabled}
          >
            <SelectTrigger id="mic-selection" className="w-full !rounded-[3px] border-[var(--pt-border-strong)] bg-[var(--pt-surface)] focus:ring-[var(--pt-accent)]">
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-[var(--pt-text-tertiary)]">No microphone devices found</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 border-t border-[var(--pt-border)] pt-2">
              <p className="text-xs font-medium text-[var(--pt-text-secondary)]">Microphone levels</p>
              {inputDevices.map((device) => {
                const levelData = audioLevels.get(device.name);
                return (
                  <div key={`level-${device.name}`} className="space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="max-w-[200px] truncate text-xs text-[var(--pt-text-secondary)]">
                        {device.name}
                      </span>
                      {levelData && (
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Speaker className="h-4 w-4 text-[var(--pt-accent)]" />
            <Label htmlFor="system-selection" className="text-sm font-medium text-[var(--pt-text)]">
              System Audio
            </Label>
          </div>
          ...
          <Select
            ...
            disabled={disabled}
          >
            <SelectTrigger id="system-selection" className="w-full !rounded-[3px] border-[var(--pt-border-strong)] bg-[var(--pt-surface)] focus:ring-[var(--pt-accent)]">
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-[var(--pt-text-tertiary)]">No system audio devices found</p>
          )}

          {/* Backend Selection - available on all platforms */}
          {!disabled && (
            <div className="border-t border-[var(--pt-border)] pt-3">
      ...
      {/* Info text */}
      <div className="space-y-1 text-xs text-[var(--pt-text-tertiary)]">
