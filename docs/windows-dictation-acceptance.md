# PulseTalq Windows dictation acceptance
...
- Settings reports the shortcut actually acquired at runtime and its Dictation tab can capture, validate, save, and immediately activate a replacement chord such as `Ctrl+Alt+D`. The preference is persisted for the next launch. Registration or persistence failure leaves the previous shortcut active and visible.
...
- Dictation settings includes a persisted floating-overlay switch. When enabled, a compact microphone stays above other windows, expands on hover to show the acquired shortcut, and remains expanded through listening, transcription, cleanup, delivery, and the brief completion or failure result. The overlay follows the pointer between monitors while idle and throughout a session. Delivery remains bound to the focused text control captured at activation, so moving the overlay never changes the paste target. Negative desktop coordinates support displays placed left of or above the primary monitor.
...
- The core dictation suite passes 43 tests; four interactive Windows tests are ignored by default because they temporarily take foreground focus or own the clipboard.
- The Pulse mark and expanded PulseTalq wordmark route Home. Each sidebar mode exposes exactly one waveform control and it routes to Dictation history; About remains available through the information control.
- The Settings shell and its General, Recording, Transcript, Summary, Dictation, and Beta surfaces use the PulseTalq Deep Focus tokens, compact borders, brand focus states, and 3 px control geometry.
- The NSIS package installs the Pulse icon beside the executable, assigns it explicitly to current-user Start menu and desktop shortcuts, repairs existing PulseTalq shortcuts on update, and only removes the transitional `PulseTalk.lnk` when it targets the old PulseTalk executable exactly. The tray implementation is unchanged.
...
7. Start dictation in a text field, move the pointer to the second monitor, and confirm the overlay follows the pointer while delivery still returns to the control captured at activation.
8. Open Settings > Dictation, capture `Ctrl+Alt+D`, save it, and confirm Home and the floating overlay update without restarting. Restart PulseTalq and confirm the chord persists.
9. Inspect the current-user Start menu and desktop shortcuts after installing or updating the candidate. Confirm they display the Pulse icon and launch the installed executable. Confirm a fresh silent install does not create a desktop shortcut when none was requested.
...
- Browser-only development previews do not provide the Tauri bridge. They can verify layout and navigation semantics, but shortcut registration, persistence, overlay movement, focused-control delivery, and installed shortcut icons require the packaged Windows app.
- The candidate installer was built and statically verified but was not installed during this change, so current-user shortcut repair remains pending in the manual matrix above.

## Current local package

- Installer: `target/release/bundle/nsis/PulseTalq_0.4.0_x64-setup.exe`
- Size: `42,891,430 bytes`
- SHA-256: `52E731BE4FDCC604FC6E7596B116CC2BA8AB6440D14DE8F2F284F9841F3F7239`
